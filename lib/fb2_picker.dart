...
class _FB2PickerState extends State<FB2Picker> {
  ...
  Future<xml.XmlDocument> _openFile() async {
    ...
    return xml.parse(contents.replaceAll('<image', '<img'));
  }
  ...
  void _openReaderV3() async {
    final document = await _openFile();
    Navigator.of(context).pushNamed(
      FB2ReaderScreenV3.pathName,
      arguments: document,
    );
  }
    ...
    return Column(
      children: <Widget>[
        ...
        if (_path != null)
          Container(
            child: RaisedButton(
              onPressed: _openReaderV5,
              child: Text("Open file in readerV5"),
            ),
          ),
      ],
    );
